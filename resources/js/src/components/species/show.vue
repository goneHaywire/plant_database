      ...
      axios
        ...
        .then((data) => {
          ...
          if (rows.length > 0) {
            ...
            document
              .querySelector(".photo-box")
              .classList.add("photo-box-image");
            document.querySelector(
              ".photo-box"
            ).style.backgroundImage = `url(${rows[0]
              .querySelector("img")
              .src.replace(/.thumb/, "")})`;
            document
              .querySelector(".photo-box-behind")
              .classList.add("photo-box-image");

            rows.forEach((row) => {
              console.log(row.querySelector("img").src);
              this.images.push(
                row.querySelector("img").src.replace(/.thumb/, "")
              );
            });
          } else {
